...
class PipelineDB:
    ...
    def _init_db(self):
        with self._connect() as conn:
            ...
            """)
            # Migración: agregar tracker_url si la tabla ya existe sin esa columna
            try:
                conn.execute("ALTER TABLE download_queue ADD COLUMN tracker_url TEXT")
    ...
    def add_vod(self, vod_id: str, channel: str, video_id: str, source: str,
                status: str = "pending", error: str = None, file_path: str = None,
                file_size_mb: float = None):
        with self._connect() as conn:
            conn.execute("""
                INSERT OR IGNORE INTO processed_vods
                (vod_id, channel, video_id, source, detected_at, status, error_message, file_path, file_size_mb)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (vod_id, channel, video_id, source,
                  datetime.now(timezone.utc).isoformat(), status, error, file_path, file_size_mb))

    def update_vod_status(self, vod_id: str, status: str, youtube_id: str = None,
                         error: str = None, file_path: str = None, file_size_mb: float = None):
        with self._connect() as conn:
    ...
    def get_vods(self, status: str = None, channel: str = None, search: str = None,
    ...
    def count_vods(self, status: str = None, channel: str = None) -> int:
        with self._connect() as conn:
            where = []
            params = []
            if status:
    ...
    def reset_vod(self, vod_id: str):
        """Pone un VOD a pending para reintentarlo."""
        with self._connect() as conn:
            conn.execute("""
                ...
                WHERE vod_id = ?
            """, (vod_id,))
            # Re-encolar si no esta
            conn.execute("""
                INSERT OR IGNORE INTO download_queue (vod_id, channel, video_id, source, start_time)
                SELECT vod_id, channel, video_id, source, 0 FROM processed_vods WHERE vod_id = ?
            """, (vod_id,))
    ...
    def enqueue(self, stream_info: dict):
        with self._connect() as conn:
            try:
                conn.execute("""
                    INSERT INTO download_queue (vod_id, channel, video_id, source, start_time, tracker_url)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (stream_info["vod_id"], stream_info["channel"],
                      stream_info["video_id"], stream_info.get("source"),
                      stream_info.get("start_time"), stream_info.get("tracker_url")))
    ...
    def dequeue(self) -> Optional[Dict]:
        with self._connect() as conn:
            ...
            if not row:
                return None
            conn.execute(
                "UPDATE download_queue SET status = 'downloading' WHERE id = ?",
                (row["id"],)
            )
            return dict(row)
    ...
    def mark_queue_status(self, vod_id: str, status: str, error: str = None):
        with self._connect() as conn:
            if status == "failed":
                ...
                conn.execute(
                    "UPDATE download_queue SET status = ? WHERE vod_id = ?",
                    (status, vod_id)
                )
