...
export function WatchVod() {
  ...
  return (
    <div>
      <PageHeader title="Watch VOD" subtitle="Play saved broadcasts with their archived chat" />
      <div className="space-y-5">
        <ReplayPlayer />
        <details className="group">
          <summary className="mb-4 cursor-pointer text-sm font-medium text-muted hover:text-fg">
            Find a recoverable video · M3U8 resolver
          </summary>
          <Card>
            <CardBody>
              <form
              ...
              <CardBody className="space-y-5">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    ...
                    {resolve.data.started_at
                      ? ` · ${new Date(resolve.data.started_at).toLocaleString()}`
                      : ""}
                  </p>
                ...
                <p className="text-xs text-muted">
                  If your browser cannot play the URL, copy it into VLC under Media → Open Network
                  Stream.
                </p>
              </CardBody>
            </Card>
